/*
 * Generation two of the HLX-M1 ELF `ET_REL` object-parsing fixture.
 *
...
/* CHANGED: the constant differs from generation one's 11. */
int hcr_lx_obj_changed_leaf(int value) { return value + 77; }

/* CHANGED: an extra call, so the relocation SIGNATURE changes and not just
 * the bytes — the diff must classify this differently from a body-only edit. */
int hcr_lx_obj_calls_external(int value) {
  return hcr_lx_obj_external(value) + hcr_lx_obj_external(value + 1);
}
