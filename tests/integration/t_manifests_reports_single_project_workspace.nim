...
##
## A ``.repo/workspace.toml`` written in single-project mode (``[workspace]``
...
## is genuinely absent:
##
##   ``workspace manifests: no layered workspace (.repo/workspace.toml not present at …)``
##
...
suite "regression — manifests reports single-project workspace accurately":
  ...
  test "t_manifests_reports_single_project_workspace":
    if not fileExists(reproBinary):
      ...
      check fileExists(ws / ".repo" / "workspace.toml")
