...
##      papers over it.
##
## ``runquota`` is a declared workspace dependency. Resolve it through the
## workspace-aware fixture helper so linked reprobuild worktrees use the
## workspace's actual checkout; a genuinely missing checkout is a hard,
## actionable fixture error rather than a skipped structural assertion.
...
suite "Bootstrap-And-Self-Build B0: runquota repro.nim compiles":
  ...
  test "../runquota/repro.nim declares the expected shape":
    ...
    let runquotaRoot = requireRunQuotaSourceRoot(reprobuildRoot)
    let runquotaRepro = runquotaRoot / "repro.nim"
    check fileExists(runquotaRepro)
