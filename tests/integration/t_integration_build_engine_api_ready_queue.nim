...
proc ensureRunQuotaDaemon(repoRoot, tempRoot: string; cpuMilli = "32000"):
  ...
  for _ in 0 ..< 200:
    # POSIX exposes a socket inode; Windows maps the same endpoint token to a
    # named pipe. Probe the protocol so this helper verifies both transports.
  ...
  daemon.close()
  raise newException(OSError, "runquotad endpoint did not become reachable")
...
suite "integration_build_engine_api_ready_queue":
  test "inline RunQuota impossible leases report a bounded deadlock":
    let repoRoot = getCurrentDir()
    let tempRoot = createTempDir("repro-inline-rq-denied", "")
    defer: removeDir(tempRoot)
    let oldRunQuotaSocket = getEnv("RUNQUOTA_SOCKET", "")
    defer:
      if oldRunQuotaSocket.len > 0:
        putEnv("RUNQUOTA_SOCKET", oldRunQuotaSocket)
      else:
        delEnv("RUNQUOTA_SOCKET")
    let oldDenialTimeout = getEnv("REPRO_RUNQUOTA_DENIAL_TIMEOUT", "")
    putEnv("REPRO_RUNQUOTA_DENIAL_TIMEOUT", "1")
    defer:
      if oldDenialTimeout.len > 0:
        putEnv("REPRO_RUNQUOTA_DENIAL_TIMEOUT", oldDenialTimeout)
      else:
        delEnv("REPRO_RUNQUOTA_DENIAL_TIMEOUT")
    ...
    try:
      discard runBuild(graph([
        ...
        inlineRunQuota: true))
    except ReproRunQuotaDeadlockError as err:
      deadlock = err.msg

    check deadlock.contains("static-capacity deadlock")
    check deadlock.contains("shared CPU budget") or
      deadlock.contains("machine CPU budget")
