...
suite "M19a — post-merge manifest auto-refresh (skips divergent layer)":
  ...
  test "test_m19a_post_merge_skips_when_manifest_layer_divergent":
    ...
    if gitBin.len == 0:
      ...
      # reconciliation that ``7700b54e`` added. "A divergent skip is quiet" is
      # a statement about the FIRST of them, so partition first and then
      # assert each half exhaustively. Widening the count to ``>= 1`` would
      # buy the same green while making the test blind to a second manifest
      # line — which is the one thing it exists to catch.
      ...
      if manifestLines.len == 1:
        check manifestLines[0].contains("skipped_divergent")
