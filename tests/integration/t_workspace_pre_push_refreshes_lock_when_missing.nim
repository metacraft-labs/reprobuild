...
##
## Lock-refresh path: a clean, fully-published workspace that has
## never been locked must still pass pre-push, AND the gate must
## create the missing lock as a side effect. The report's
## ``lockUpdate.kind`` is ``created`` (not ``already-current``) and
## the on-disk lock TOML + index TOML appear under the manifest layer.
##
...
proc setupFixture(gitBin, slug: string): M18Fixture =
  ...
  # This suite asserts the gate REFRESHES a manifest lock record, which only
  ...
  # that never declares one is public-only and writes only `repro.lock`). Make
  # `.repro/manifests` a real git checkout so the route is DECLARED rather than
  # inferred from a path — previously the gate synthesized the store
  # unconditionally, so this fixture got a lock record it had never asked for.
...
suite "M18 — repro check --mode=pre-push (lock-refresh path)":

  test "t_workspace_pre_push_refreshes_lock_when_missing":
    let gitBin = findExe("git")
    if gitBin.len == 0:
      skip()
    else:
      let fx = setupFixture(gitBin, "lock-missing")
