...
class DevEnvM9PolicyTest(unittest.TestCase):
    ...
    def test_full_suite_gate_keeps_relevant_components_in_scope(self):
        ...
        # Project-DSL-Composition M6 migration: ``./build/bin/repro build test``
        # walks the ``reprobuildTestSpecs`` data table in repro.nim's ``build:``
        # block (declared in ``repro_tests.nim``, regenerated by
        # ``scripts/generate_test_edges.nim``). The old shell ``find tests
        # -name 't*.nim'`` discovery is retired. Enforce the new entry point
        # instead — the data-table approach must be the live driver, not a
        # branch the shim happens to also have.
        self.assertIn("./build/bin/repro build test", run_tests)
