...
class SuiteCaseCountPushGateTests(unittest.TestCase):
    ...
    def populate(self, root):
        ...
        wanted = [
            ...
            # The shape-parity artifact. The gate refuses to run without it --
            # a parity check that passes when it has nothing to compare
            # against proves nothing -- so a scratch tree that omitted it
            # would fail for a fixture reason rather than a real one.
            #
            # It is copied rather than regenerated, and that is not a
            # shortcut: it is regenerated by `nim r
            # scripts/generate_test_edges.nim`, which this tree deliberately
            # cannot run, and it never needs to be. It goes stale when the
            # RULE changes, not when the tree does, and both sides of the rule
            # are copied here verbatim. `regenerate()` below therefore still
            # runs only the two commands the refusal actually prints.
            inventory.SHAPE_PARITY_PATH,
        ]
...
class ShapeParityGateTests(unittest.TestCase):
    """One rule, two implementations, agreement by measurement.

    `_is_test_shaped_source` in `scripts/reprobuild_suite_inventory.py` and
    `isTestShapedSource` in `scripts/generate_test_edges.nim` decide the same
    question in two languages. Nothing about writing the same words twice
    makes them stay the same words, and while they disagree one of two things
    is true, both bad: the undeclared-source gate names an orphan that
    regenerating cannot enrol -- a gate demanding a fix the contributor cannot
    perform -- or a test is built and run that the gate never audits.

    They agreed on the tree as it stood and on nothing else.
    `tests/unit/test_foo.nim` and `libs/<name>/<sub>/tests/t_foo.nim` were both
    accepted here and refused there; neither existed, and nothing stopped one
    being added.

    Every case below is negative-first: each asserts something that is FALSE
    when the machinery is broken in the specific way the case names.
    """

    def test_the_corpus_is_a_real_product_of_its_four_lists(self):
        """A corpus that silently shrank would silently compare less.

        The check's whole strength is the SET it compares, so the set is
        pinned to its own construction rather than to a number: it is the
        cartesian product of the four lists, with no duplicates.
        """
        corpus = inventory.shape_parity_corpus()
        expected = (
            len(inventory.SHAPE_CORPUS_ROOTS)
            * len(inventory.SHAPE_CORPUS_MIDDLES)
            * len(inventory.SHAPE_CORPUS_STEMS)
            * len(inventory.SHAPE_CORPUS_EXTS)
        )
        self.assertEqual(len(corpus), expected)
        self.assertEqual(len(set(corpus)), expected, "the corpus has duplicates")

    def test_the_corpus_exercises_both_answers_on_every_axis(self):
        """A corpus every row of which answers the same way compares nothing.

        This is the vacuity that would make the whole check green and
        worthless, so it is asserted directly rather than inferred from the
        gate passing.
        """
        corpus = inventory.shape_parity_corpus()
        accepted = [r for r in corpus if inventory.gate_accepts_source(r)]
        refused = [r for r in corpus if not inventory.gate_accepts_source(r)]
        self.assertTrue(accepted, "no corpus row is accepted")
        self.assertTrue(refused, "no corpus row is refused")
        # And the axes the two rules actually diverged on are present. A
        # tautological membership assertion here would be exactly the vacuity
        # this case exists to rule out, so each is a real search of `accepted`
        # or `refused`.
        self.assertTrue(
            any(r.endswith("/test_x.nim") and r.startswith("tests/") for r in accepted),
            "the `tests/` + `test_` stem shape is not exercised",
        )
        self.assertTrue(
            any("/src/tests/" in r and r.endswith("/t_x.nim") for r in accepted),
            "a tests directory below src/ is not exercised",
        )
        self.assertTrue(
            any(r.startswith("tests/fixtures/") for r in refused),
            "the shared fixture skip prefix is not exercised",
        )

    def test_the_recorded_artifact_agrees_with_this_files_rule(self):
        """The gate itself, on the checked-in artifact."""
        drift = inventory.shape_parity_drift(REPO_ROOT)
        self.assertTrue(drift["clean"], inventory.format_shape_parity_drift(drift))
        self.assertFalse(drift["degenerate"])

    def test_a_disagreement_is_reported_and_named(self):
        """Negative control: the check can fail, and says which path.

        Written against a fabricated artifact rather than the real one so the
        assertion does not depend on ever breaking the tree.
        """
        root = Path(tempfile.mkdtemp(prefix="shape-parity-"))
        self.addCleanup(shutil.rmtree, root, ignore_errors=True)
        target = root / inventory.SHAPE_PARITY_PATH
        target.parent.mkdir(parents=True, exist_ok=True)
        rows = []
        flipped = None
        for rel in inventory.shape_parity_corpus():
            verdict = "ACCEPT" if inventory.gate_accepts_source(rel) else "REJECT"
            if flipped is None and verdict == "ACCEPT":
                flipped = rel
                verdict = "REJECT"
            rows.append(f"{rel}\t{verdict}")
        target.write_text("\n".join(rows) + "\n", encoding="utf-8")

        drift = inventory.shape_parity_drift(root)
        self.assertFalse(drift["clean"])
        self.assertEqual(len(drift["disagreements"]), 1)
        self.assertEqual(drift["disagreements"][0][0], flipped)
        self.assertIn(flipped, inventory.format_shape_parity_drift(drift))

    def test_a_drifted_corpus_says_it_proves_nothing(self):
        """Negative control, and the one that matters most.

        If the two sides stop describing the same corpus, the rows being
        compared are not the rows either side thinks it is classifying. That
        must read as "this run proves nothing", never as agreement -- the
        exact failure a sibling gate was rewritten to avoid.
        """
        root = Path(tempfile.mkdtemp(prefix="shape-parity-"))
        self.addCleanup(shutil.rmtree, root, ignore_errors=True)
        target = root / inventory.SHAPE_PARITY_PATH
        ...
        corpus = inventory.shape_parity_corpus()
        rows = [
            f"{rel}\t{'ACCEPT' if inventory.gate_accepts_source(rel) else 'REJECT'}"
            for rel in corpus[:-1]
        ]
        target.write_text("\n".join(rows) + "\n", encoding="utf-8")
        ...
        drift = inventory.shape_parity_drift(root)
        self.assertFalse(drift["clean"])
        self.assertEqual(drift["corpusOnlyHere"], [corpus[-1]])
        self.assertIn("proves nothing", inventory.format_shape_parity_drift(drift))

    def test_a_degenerate_corpus_is_a_failure_not_a_pass(self):
        """Negative control: uniform verdicts compare nothing.

        An artifact that says ACCEPT for every row would produce zero
        disagreements against a gate that also accepted everything. That is a
        green off a comparison that was never made, so it is refused.
        """
        root = Path(tempfile.mkdtemp(prefix="shape-parity-"))
        self.addCleanup(shutil.rmtree, root, ignore_errors=True)
        target = root / inventory.SHAPE_PARITY_PATH
        ...
        corpus = inventory.shape_parity_corpus()
        target.write_text(
            "\n".join(f"{rel}\tACCEPT" for rel in corpus) + "\n", encoding="utf-8"
        )
        with mock.patch.object(inventory, "gate_accepts_source", return_value=True):
            drift = inventory.shape_parity_drift(root)
        self.assertFalse(drift["clean"])
        self.assertTrue(drift["degenerate"])
        self.assertEqual(drift["disagreements"], [])
        self.assertIn("degenerate", inventory.format_shape_parity_drift(drift))

    def test_a_missing_artifact_is_fatal_rather_than_skipped(self):
        """Negative control: an absent input must not report green.

        A check that passes when the artifact it guards is absent proves
        nothing, and is how a stale baseline reaches a branch.
        """
        root = Path(tempfile.mkdtemp(prefix="shape-parity-"))
        self.addCleanup(shutil.rmtree, root, ignore_errors=True)
        drift = inventory.shape_parity_drift(root)
        ...
        self.assertIn("not found", inventory.format_shape_parity_drift(drift))

    def test_the_generator_no_longer_carries_a_narrower_shape_rule(self):
        """The structural half of the fix, asserted in the generator's source.

        The five per-root accept predicates are what the gate could be wider
        than. With them gone there is no narrower rule left for the two to
        disagree about, and this case pins that: if someone reintroduces one,
        the corpus may or may not happen to contain a path that exposes it,
        but this fails immediately.
        """
        text = (REPO_ROOT / "scripts" / "generate_test_edges.nim").read_text(
            encoding="utf-8"
        )
        for gone in (
            "proc acceptTestsTree",
            "proc acceptLibsTree",
            "proc acceptToolsTree",
            "proc acceptRecipesTree",
            "proc acceptAppsTree",
        ):
            self.assertNotIn(gone, text, f"{gone} is back; the rules can diverge again")
        self.assertIn("proc isTestShapedSource", text)
        self.assertIn("proc walkAcceptsSource", text)
        # The roots the two sides walk must be the same roots.
        for root in inventory.DECLARED_SOURCE_ROOTS:
            self.assertIn(f'"{root}"', text)

    def test_the_push_gate_runs_it(self):
        """Wiring, asserted at the call site rather than assumed.

        A check nothing invokes is not a gate.
        """
        text = (REPO_ROOT / "scripts" / "check_suite_case_counts.sh").read_text(
            encoding="utf-8"
        )
        self.assertIn("--check-shape-parity", text)
        self.assertIn("parity_status", text)
        justfile = (REPO_ROOT / "Justfile").read_text(encoding="utf-8")
        self.assertIn("--check-shape-parity", justfile)
