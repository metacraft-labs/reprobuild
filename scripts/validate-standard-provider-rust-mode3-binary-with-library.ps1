#requires -Version 5
# End-to-end verification: build the rust-mode3/binary-with-library
...
#   2. Probe for rustc (falling back to the bundled rustup stable
#      toolchain under D:/metacraft-dev-deps/rustup/); SKIP exit 0 if
...
#   7. Locate the produced ``calc[.exe]`` under
#      <fixture>/.repro/build/calc/ and run it; assert stdout contains
#      ``hello from rust-mode3-binary-with-library, mathlib added 2+3 = 5``.
#   8. Also verify the upstream rlib ``libmathlib.rlib`` was produced
#      alongside.
#
# Mode 3 Rust contract (M30 of Mode3-Language-Expansion.milestones.org):
# the convention emits the library rustc action FIRST (sequenced via
# depends_on calcPkg: mathlibPkg) and the executable's rustc link argv
# carries ``--extern mathlib=<rlib>`` so ``use mathlib::add;`` in
# calc/src/main.rs resolves. This script is the load-bearing
# end-to-end gate that turns the convention's action graph into a
# working executable.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\rust-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'calc'
$libArchive     = Join-Path $fixture '.repro\build\mathlib\libmathlib.rlib'
$expectedGreeting = 'hello from rust-mode3-binary-with-library, mathlib added 2+3 = 5'
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-rust-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-rust-mode3-binary-with-library.stderr.txt'
...
Write-Host ""
Write-Host "PASS: rust-mode3/binary-with-library built via standard provider; greeting matched; libmathlib.rlib produced"
exit 0
