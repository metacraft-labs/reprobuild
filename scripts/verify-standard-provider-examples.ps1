...
function Get-ExpectedOutputs([string]$rel, [string]$fixtureDir) {
  switch ($rel) {
    ...
    'nim/mode3-pilot' {
      ...
      # curated `repro.nim`"). The fixture declares a `library greet`
      # AND `executable hello` in a single `package mode3Pilot:`. The
      # executable's source imports the library's umbrella module; the
      # build produces both a `libgreet.a` static archive AND a
      # `hello.exe` whose greeting confirms the inter-member link
      # closed end-to-end. The companion `repro.scanned-deps.nim`
      # (generated by `repro deps refresh`) is empty today because both
      # members live in the SAME package — multi-package edges land in
      # a follow-on milestone, see the fixture's `repro.nim` header.
      return @(
        @{
