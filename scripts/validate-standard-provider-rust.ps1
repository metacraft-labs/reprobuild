#requires -Version 5
# End-to-end M4 verification: build the Rust binary example via the
# Tier 2b dispatch path and run the produced executable.
#
# Mechanics:
#
#   1. Source D:/metacraft/env.ps1 so the managed nim/gcc/repro tools are
#      on PATH. The dev shell does NOT (yet) provision rustc/cargo;
#      if neither is on PATH after sourcing, fall back to the rustup
#      stable toolchain under D:/metacraft-dev-deps/rustup/toolchains.
#      If that's also missing the script exits with SKIP=0 (informational
#      success) so the milestone gate is honest about toolchain absence.
...
#   6. Locate the produced binary under <fixture>/.repro/build/<crate>/bin/
...
# §M4 verification "e2e_rust_binary_builds_via_standard_provider".
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\rust\binary'
$scratchInsideFixture = Join-Path $fixture '.repro'
$crateName      = 'rust_binary_example'
$expectedBinary = Join-Path $fixture (Join-Path '.repro\build' (Join-Path $crateName (Join-Path 'bin' ($crateName + '.exe'))))
$expectedGreeting = "hello from rust-binary-example"
...
if (-not $rustcCmd -or -not $cargoCmd) {
  Write-Host "SKIP: rustc/cargo not available — the M4 e2e gate needs both on PATH."
...
# Also wipe any leftover ./target dir from manual rustc smoke tests.
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-rust.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-rust.stderr.txt'
...
Write-Host ""
Write-Host "PASS: rust/binary built via standard provider; greeting matched"
exit 0
