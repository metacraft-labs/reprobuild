#requires -Version 5
# End-to-end verification: build the zig-mode3/binary-with-library
...
#   2. Probe for zig (PATH or D:/metacraft-dev-deps/zig/<v>/); SKIP exit
#      0 if missing.
...
#   7. Locate the produced ``zigcalc[.exe]`` under
#      <fixture>/.repro/build/zigcalc/ and run it; assert stdout
#      contains the expected greeting.
#   8. Also verify the upstream library archive ``libziglib.a`` was
#      produced.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\zig-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'zigcalc'
$libArchive     = Join-Path $fixture '.repro\build\ziglib\libziglib.a'
$expectedGreeting = 'hello from zig-mode3-binary-with-library, ziglib added 2+3 = 5'
...
if (-not $zig) {
  Write-Host "SKIP: 'zig' not on PATH and not under D:/metacraft-dev-deps/zig/ (download from ziglang.org)"
  exit 0
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-zig-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-zig-mode3-binary-with-library.stderr.txt'
...
Write-Host ""
Write-Host "PASS: zig-mode3/binary-with-library built via standard provider; greeting matched; libziglib.a produced"
exit 0
