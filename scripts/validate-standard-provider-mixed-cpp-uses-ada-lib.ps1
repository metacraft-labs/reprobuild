#requires -Version 5
# End-to-end verification: build the mixed/cpp-uses-ada-lib fixture
# (M58 reverse cross-language direction: C++ binary calling an Ada
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\cpp-uses-ada-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'cppapp'
$libArchive     = Join-Path $fixture '.repro\build\adaaddlib\libadaaddlib.a'
$expectedGreeting = 'cpp says: ada added 2+3 = 5'
...
if (-not $gnatmake) {
  Write-Host "SKIP: 'gnatmake' not on PATH (install via 'pacman -S mingw-w64-x86_64-gcc-ada' under MSYS2; M58 reverse fixture needs Ada)"
  exit 0
}
$cxx = Get-Command g++ -ErrorAction SilentlyContinue
if (-not $cxx) { $cxx = Get-Command clang++ -ErrorAction SilentlyContinue }
if (-not $cxx) {
  Write-Host "SKIP: neither 'g++' nor 'clang++' on PATH (M58 reverse fixture needs a C++ link driver)"
  exit 0
}
$ar = Get-Command ar -ErrorAction SilentlyContinue
if (-not $ar) {
  Write-Host "SKIP: 'ar' not on PATH (M58 reverse fixture needs an archiver)"
  exit 0
}
Write-Host "==> using gnatmake=$($gnatmake.Source); cxx=$($cxx.Source); ar=$($ar.Source)"
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-ada-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-ada-lib.stderr.txt'
...
# --- step 4: verify upstream Ada archive exists ---
if (-not (Test-Path -LiteralPath $libArchive)) {
  Write-Host "FAIL: expected upstream Ada archive not found at $libArchive"
  exit 1
}
...
Write-Host ""
Write-Host "PASS: mixed/cpp-uses-ada-lib built; greeting matched; libadaaddlib.a produced; cross-lang C++ -> Ada round-trip"
exit 0
