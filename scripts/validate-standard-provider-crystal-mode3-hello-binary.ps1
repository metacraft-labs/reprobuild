#requires -Version 5
# End-to-end M60 verification: build the crystal-mode3/hello-binary
# example via the Tier 2b dispatch path and run the produced binary.
#
# Mechanics:
#
#   1. Source D:/metacraft/env.ps1 so repro.exe is on PATH.
#   2. Probe for ``crystal`` only (Mode 3 bypasses ``shards`` — no
#      ``shard.yml`` at the workspace root). SKIP exit 0 if missing.
...
#      ``hello from crystal-mode3-hello-binary``.
#
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\crystal-mode3\hello-binary'
$scratchInsideFixture = Join-Path $fixture '.repro'
$expectedGreeting = 'hello from crystal-mode3-hello-binary'
...
if (Test-Path -LiteralPath (Join-Path $fixture 'shard.yml')) {
  Write-Host "FAIL: crystal-mode3 fixture must NOT have shard.yml at $fixture"
  exit 1
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-crystal-mode3-hello-binary.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-crystal-mode3-hello-binary.stderr.txt'
...
Write-Host ""
Write-Host "PASS: crystal-mode3/hello-binary built via standard provider; greeting matched"
exit 0
