#requires -Version 5
# End-to-end verification: build the go-mode3/binary-with-library
...
#   2. Probe for go (falling back to the bundled toolchain under
#      D:/metacraft-dev-deps/go/); SKIP exit 0 if go is unavailable.
...
#      ``hello from go-mode3-binary-with-library, mathlib added 2+3 = 5``.
#   8. Also verify the upstream library archive ``mathlib.a`` was
#      produced alongside.
#
# Mode 3 Go contract (M31 of Mode3-Language-Expansion.milestones.org):
# the convention emits the library compile action FIRST (sequenced via
# depends_on calcPkg: mathlibPkg) and the executable's compile + link
# argv references the upstream archive via importcfg / importcfg.link.
# This script is the load-bearing end-to-end gate that turns the
# convention's action graph into a working executable.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\go-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'calc'
$libArchive     = Join-Path $fixture '.repro\build\mathlib\mathlib.a'
$expectedGreeting = 'hello from go-mode3-binary-with-library, mathlib added 2+3 = 5'
...
if (-not $goCmd) {
  Write-Host "SKIP: 'go' not on PATH and not under D:/metacraft-dev-deps/go/"
  exit 0
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-go-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-go-mode3-binary-with-library.stderr.txt'
...
# --- step 4: verify library archive exists ---
...
Write-Host ""
Write-Host "PASS: go-mode3/binary-with-library built via standard provider; greeting matched; mathlib.a produced"
exit 0
