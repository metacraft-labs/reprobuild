...
# C++ binary calls into a Fortran static library declared in the same
# repro.nim (REVERSE direction). The fortran-direct convention emits the
# Fortran archive AND the C++ binary's per-source compile + terminal link
# with explicit Fortran runtime libs (``-lgfortran -lquadmath -lm`` plus
# ``-lpthread`` on POSIX) so the C++ link resolves Fortran runtime symbols.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\cpp-uses-fortran-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$libArchive     = Join-Path $fixture '.repro\build\fortaddlib\libfortaddlib.a'
$expectedAddition = 'cpp says: fortran added 2+3 = 5'
$expectedGreeting = 'hello from cpp-uses-fortran-lib'
...
if (-not $gpp -and -not $clangpp) {
  Write-Host "SKIP: neither g++ nor clang++ on PATH; cannot build mixed cpp-uses-fortran-lib"
  exit 0
}
$ar = Get-Command ar -ErrorAction SilentlyContinue
if (-not $ar) {
  Write-Host "SKIP: 'ar' not on PATH; cannot archive the upstream Fortran library"
  exit 0
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-fortran-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-fortran-lib.stderr.txt'
...
# --- step 6: run and assert greeting + Fortran call result ---
...
if ($output -notmatch [regex]::Escape($expectedAddition)) {
  Write-Host "FAIL: produced binary stdout does not contain expected addition output '$expectedAddition' (this proves the Fortran archive's fortran_add symbol is linked AND callable from C++ via the bind(C) interface)"
  exit 1
}
...
Write-Host ""
Write-Host "PASS: mixed/cpp-uses-fortran-lib built via standard provider; cross-language call to Fortran fortran_add() produced cpp says: fortran added 2+3 = 5; libfortaddlib.a archive produced"
exit 0
