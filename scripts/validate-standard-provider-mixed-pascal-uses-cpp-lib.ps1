#requires -Version 5
# End-to-end verification: build the mixed/pascal-uses-cpp-lib fixture
# (M59 forward cross-language direction: Pascal binary calling a C
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\pascal-uses-cpp-lib'
...
$expectedGreeting = 'pascal says: mathlib added 2+3 = 5'
...
if (-not $fpc) {
  Write-Host "SKIP: 'fpc' not on PATH (install via 'pacman -S mingw-w64-x86_64-fpc' under MSYS2 or download from freepascal.org; M59 forward fixture needs Pascal)"
  exit 0
}
$cc = Get-Command gcc -ErrorAction SilentlyContinue
if (-not $cc) { $cc = Get-Command clang -ErrorAction SilentlyContinue }
if (-not $cc) {
  Write-Host "SKIP: neither 'gcc' nor 'clang' on PATH (M59 forward fixture needs a C compiler)"
  exit 0
}
$ar = Get-Command ar -ErrorAction SilentlyContinue
if (-not $ar) {
  Write-Host "SKIP: 'ar' not on PATH (M59 forward fixture needs an archiver)"
  exit 0
}
Write-Host "==> using fpc=$($fpc.Source); cc=$($cc.Source); ar=$($ar.Source)"
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-pascal-uses-cpp-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-pascal-uses-cpp-lib.stderr.txt'
...
Write-Host ""
Write-Host "PASS: mixed/pascal-uses-cpp-lib built; greeting matched; libmathlib.a produced; cross-lang Pascal -> C round-trip"
exit 0
