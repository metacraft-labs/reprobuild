#requires -Version 5
# End-to-end verification: build the c-cpp-mode3/binary-with-library
...
#      ``hello from c-cpp-mode3-binary-with-library, 2 + 3 = 5``.
#   8. Also verify the upstream library archive ``libmathlib.a`` was
#      produced alongside.
#
# Mode 3 contract: the convention emits the archive action FIRST
# (sequenced via depends_on calcPkg: mathlibPkg) and the link action's
# argv carries the archive path as a trailing positional. This script
# is the load-bearing end-to-end gate that turns the convention's
# action graph into a working executable.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\c-cpp-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'calc'
$libArchive     = Join-Path $fixture '.repro\build\mathlib\libmathlib.a'
$expectedGreeting = 'hello from c-cpp-mode3-binary-with-library, 2 + 3 = 5'
...
if (-not $gcc -and -not $clang) {
  Write-Host "SKIP: neither gcc nor clang on PATH; cannot build c-cpp-mode3/binary-with-library"
  exit 0
}
if ($gcc) {
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-c-cpp-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-c-cpp-mode3-binary-with-library.stderr.txt'
...
Write-Host ""
Write-Host "PASS: c-cpp-mode3/binary-with-library built via standard provider; greeting matched; libmathlib.a produced"
exit 0
