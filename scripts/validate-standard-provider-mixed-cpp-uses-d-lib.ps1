#requires -Version 5
# End-to-end verification: build the mixed/cpp-uses-d-lib fixture
# (M45 cross-language REVERSE direction: C++ binary -> D archive)
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\cpp-uses-d-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'cppapp'
$libArchive     = Join-Path $fixture '.repro\build\daddlib\libdaddlib.a'
$expectedGreeting = 'cpp says: d added 2+3 = 5'
...
if (-not $dCompiler) {
  Write-Host "SKIP: no D compiler (ldmd2/dmd/ldc2) on PATH and not under D:/metacraft-dev-deps/ldc/ (M45 reverse fixture needs D)"
  exit 0
}
$cxx = Get-Command g++ -ErrorAction SilentlyContinue
if (-not $cxx) { $cxx = Get-Command clang++ -ErrorAction SilentlyContinue }
if (-not $cxx) {
  Write-Host "SKIP: neither 'g++' nor 'clang++' on PATH (M45 reverse fixture needs a C++ link driver)"
  exit 0
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-d-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-d-lib.stderr.txt'
...
Write-Host ""
Write-Host "PASS: mixed/cpp-uses-d-lib built via standard provider; greeting matched; libdaddlib.a produced"
exit 0
