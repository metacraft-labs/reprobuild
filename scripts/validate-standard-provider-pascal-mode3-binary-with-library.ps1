#requires -Version 5
# End-to-end verification: build the pascal-mode3/binary-with-library
...
#   2. Probe for fpc on PATH; SKIP exit 0 if missing.
...
#   7. Locate the produced ``pascalcalc[.exe]`` under
#      <fixture>/.repro/build/pascalcalc/ and run it; assert stdout
#      contains the expected greeting.
#   8. Also verify the upstream library archive ``libpascallib.a``
#      was produced.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\pascal-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'pascalcalc'
$libArchive     = Join-Path $fixture '.repro\build\pascallib\libpascallib.a'
$expectedGreeting = 'hello from pascal-mode3-binary-with-library, pascallib added 2+3 = 5'
...
# --- toolchain probe ---
# M59 honest-scope cut: env.ps1 doesn't yet provision the FPC
# (mingw-w64-x86_64-fpc) toolchain so most hosts SKIP this gate
# cleanly. windows/ensure-fpc.ps1 + a Phase 1 provisioning milestone
# will lift this.
$fpc = Get-Command fpc -ErrorAction SilentlyContinue
if (-not $fpc) {
  Write-Host "SKIP: 'fpc' not on PATH (install via 'pacman -S mingw-w64-x86_64-fpc' under MSYS2 or download from freepascal.org)"
  exit 0
}
Write-Host "==> using fpc=$($fpc.Source)"
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-pascal-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-pascal-mode3-binary-with-library.stderr.txt'
...
Write-Host ""
Write-Host "PASS: pascal-mode3/binary-with-library built via standard provider; greeting matched; libpascallib.a produced"
exit 0
