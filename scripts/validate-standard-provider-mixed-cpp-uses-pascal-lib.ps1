#requires -Version 5
# End-to-end verification: build the mixed/cpp-uses-pascal-lib fixture
# (M59 reverse cross-language direction: C++ binary calling a Pascal
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\cpp-uses-pascal-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'cppapp'
$libArchive     = Join-Path $fixture '.repro\build\pascaladdlib\libpascaladdlib.a'
$expectedGreeting = 'cpp says: pascal added 2+3 = 5'
...
# --- toolchain probe (Pascal + C++) ---
$fpc = Get-Command fpc -ErrorAction SilentlyContinue
if (-not $fpc) {
  Write-Host "SKIP: 'fpc' not on PATH (install via 'pacman -S mingw-w64-x86_64-fpc' under MSYS2 or download from freepascal.org; M59 reverse fixture needs Pascal)"
  exit 0
}
$cxx = Get-Command g++ -ErrorAction SilentlyContinue
if (-not $cxx) { $cxx = Get-Command clang++ -ErrorAction SilentlyContinue }
if (-not $cxx) {
  Write-Host "SKIP: neither 'g++' nor 'clang++' on PATH (M59 reverse fixture needs a C++ link driver)"
  exit 0
}
$ar = Get-Command ar -ErrorAction SilentlyContinue
if (-not $ar) {
  Write-Host "SKIP: 'ar' not on PATH (M59 reverse fixture needs an archiver)"
  exit 0
}
Write-Host "==> using fpc=$($fpc.Source); cxx=$($cxx.Source); ar=$($ar.Source)"
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-pascal-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-pascal-lib.stderr.txt'
...
# --- step 4: verify upstream Pascal archive exists ---
if (-not (Test-Path -LiteralPath $libArchive)) {
  Write-Host "FAIL: expected upstream Pascal archive not found at $libArchive"
  exit 1
}
...
Write-Host ""
Write-Host "PASS: mixed/cpp-uses-pascal-lib built; greeting matched; libpascaladdlib.a produced; cross-lang C++ -> Pascal round-trip"
exit 0
