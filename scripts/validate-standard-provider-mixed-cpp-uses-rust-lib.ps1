#requires -Version 5
# End-to-end verification: M34 cross-language Mode 3 workspace where a
# C++ binary calls into a Rust static library declared in the same
# repro.nim (REVERSE direction). Closes the loop opened by
# validate-standard-provider-mixed-rust-uses-cpp-lib.ps1 (which routes
# Rust -> C); this script routes C++ -> Rust.
#
# The rust-direct convention takes ownership of dispatch because
# c-cpp-direct's recognize defers when the workspace's ``uses:`` block
# names ``rust`` (and no ``Cargo.toml`` is present). It then:
#
#   1. Emits the Rust library archive for ``addlib`` with
#      ``--crate-type=staticlib`` (NOT rlib) because addlib is marked
#      ``cConsumable=true`` (derived from ``depends_on cppapp: addlib``).
#      The archive contains the user-exported ``rust_add`` symbol (via
#      ``#[no_mangle] pub extern "C"``); the library uses ``#![no_std]``
#      + ``-C panic=abort`` to keep the archive small and avoid
#      MSVC-rustc + MinGW-gcc ABI mismatches.
...
#      MinGW: ``-lws2_32 -luserenv -ladvapi32 -lbcrypt -lntdll``). Build
#      sequencing strictly orders archive before link.
#
# Mechanics:
#
#   1. Source D:/metacraft/env.ps1 so nim/g++/repro are on PATH; prepend
#      the rustup stable bin so rustc resolves.
#   2. Probe for rustc AND g++ (with fallback to clang++); SKIP exit 0
#      if either toolchain is missing.
...
#         "cpp says: rust added 2+3 = 5"
#         "hello from cpp-uses-rust-lib"
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\cpp-uses-rust-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$libArchive     = Join-Path $fixture '.repro\build\addlib\libaddlib.a'
$expectedAddition = 'cpp says: rust added 2+3 = 5'
$expectedGreeting = 'hello from cpp-uses-rust-lib'
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-rust-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-cpp-uses-rust-lib.stderr.txt'
...
if ($output -notmatch [regex]::Escape($expectedAddition)) {
  Write-Host "FAIL: produced binary stdout does not contain expected addition output '$expectedAddition' (this proves the Rust archive's rust_add() symbol is linked AND callable from C++)"
  exit 1
}
...
Write-Host ""
Write-Host "PASS: mixed/cpp-uses-rust-lib built via standard provider; cross-language call from C++ to Rust rust_add() produced 2 + 3 = 5; libaddlib.a archive produced (staticlib via cConsumable flag)"
exit 0
