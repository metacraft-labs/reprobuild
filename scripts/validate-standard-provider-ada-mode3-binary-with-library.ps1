#requires -Version 5
# End-to-end verification: build the ada-mode3/binary-with-library
...
#   2. Probe for gnatmake on PATH; SKIP exit 0 if missing.
...
#   7. Locate the produced ``adacalc[.exe]`` under
#      <fixture>/.repro/build/adacalc/ and run it; assert stdout
#      contains the expected greeting.
#   8. Also verify the upstream library archive ``libadalib.a`` was
#      produced.
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\ada-mode3\binary-with-library'
$scratchInsideFixture = Join-Path $fixture '.repro'
$memberName     = 'adacalc'
$libArchive     = Join-Path $fixture '.repro\build\adalib\libadalib.a'
$expectedGreeting = 'hello from ada-mode3-binary-with-library, adalib added 2+3 = 5'
...
# --- toolchain probe ---
# M58 honest-scope cut: env.ps1 doesn't yet provision the GNAT
# (mingw-w64-x86_64-gcc-ada) toolchain so most hosts SKIP this gate
# cleanly. windows/ensure-gnat.ps1 + a Phase 1 provisioning milestone
# will lift this.
$gnatmake = Get-Command gnatmake -ErrorAction SilentlyContinue
if (-not $gnatmake) {
  Write-Host "SKIP: 'gnatmake' not on PATH (install via 'pacman -S mingw-w64-x86_64-gcc-ada' under MSYS2)"
  exit 0
}
Write-Host "==> using gnatmake=$($gnatmake.Source)"
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-ada-mode3-binary-with-library.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-ada-mode3-binary-with-library.stderr.txt'
...
Write-Host ""
Write-Host "PASS: ada-mode3/binary-with-library built via standard provider; greeting matched; libadalib.a produced"
exit 0
