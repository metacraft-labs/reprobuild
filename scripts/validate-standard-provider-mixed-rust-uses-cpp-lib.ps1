...
# Rust binary calls into a C static library declared in the same
# repro.nim (FORWARD direction). The rust-direct convention takes
# ownership of dispatch because c-cpp-direct's recognize defers when
# the workspace's ``uses:`` block names ``rust`` / ``rustc`` (and no
# ``Cargo.toml`` is present). It then emits the C/C++ archive actions
# for ``uses: gcc/clang`` packages in-line and wires them into the
# Rust binary's rustc link argv as
# ``-L native=<dir>`` + ``-l static=<libname>``.
#
# Mechanics:
#
#   1. Source D:/metacraft/env.ps1 so nim/gcc/repro are on PATH; prepend
#      the rustup stable bin so rustc resolves.
#   2. Probe for rustc AND gcc (with fallback to clang); SKIP exit 0 if
...
#       * the Rust binary  : ``<fixture>/.repro/build/calc/calc[.exe]``
#   8. Run the Rust binary and assert stdout contains:
#         "rust says: mathlib added 2+3 = 5"
#         "hello from rust-uses-cpp-lib"
#      The first line proves the C archive's ``add`` symbol is linked
#      AND callable from Rust via the ``extern "C"`` block (the cross-
#      language round-trip succeeded).
...
$fixture        = Join-Path $metacraftRoot 'reprobuild-examples\mixed\rust-uses-cpp-lib'
$scratchInsideFixture = Join-Path $fixture '.repro'
$libArchive     = Join-Path $fixture '.repro\build\mathlib\libmathlib.a'
$expectedAddition = 'rust says: mathlib added 2+3 = 5'
$expectedGreeting = 'hello from rust-uses-cpp-lib'
...
if (-not $gcc -and -not $clang) {
  Write-Host "SKIP: neither gcc nor clang on PATH; cannot build mixed rust-uses-cpp-lib"
  exit 0
}
...
$stdoutCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-rust-uses-cpp-lib.stdout.txt'
$stderrCapture = Join-Path $repoRoot 'build\validate-standard-provider-mixed-rust-uses-cpp-lib.stderr.txt'
...
if ($output -notmatch [regex]::Escape($expectedAddition)) {
  Write-Host "FAIL: produced binary stdout does not contain expected addition output '$expectedAddition' (this proves the C archive's add() symbol is linked AND callable from Rust)"
  exit 1
}
...
Write-Host ""
Write-Host "PASS: mixed/rust-uses-cpp-lib built via standard provider; cross-language call to C add() produced rust says: mathlib added 2+3 = 5; libmathlib.a archive produced"
exit 0
