# The reprobuild binary-cache HTTP server as a NixOS module.
#
# Moved here from `metacraft-labs/nixos-modules` (`modules/mcl-repro-binary-cache`)
# for Distribution-And-Packaging M4 (§9): the flake exports it as
# `nixosModules.repro-binary-cache`, so an external Nix user can host the cache
# without depending on nixos-modules. nixos-modules now re-exports THIS
# definition; there is no second copy.
#
# The option path (`services.mcl-repro-binary-cache`) and the systemd unit name
# (`mcl-repro-binary-cache.service`) are deliberately UNCHANGED by the move.
# Nothing in M4 requires renaming them, and both are load-bearing for already
# deployed hosts and for the three nixos-modules VM checks that wait on the
# unit by name; a cosmetic rename here would be churn with a real blast radius.
# (`services.reprobuild` in ./reprobuild.nix IS renamed, because M4's gate names
# that path.)
#
# `defaultPackageFor` is supplied by the caller: `system: <the
# repro-binary-cache package for that system>`. reprobuild's own flake passes
# `self.packages.${system}.repro-binary-cache`.
{ defaultPackageFor }:
...
let
  ...
  # Indexing an output attrset directly rather than going through
  # flake-parts' `withSystem` is deliberate: `withSystem` reads
  # `config.allSystems`, which -- when this module is forced through
  # `top.config.flake` inside a `perSystem` check -- creates a
  # perSystem->flake->perSystem eval cycle.
  defaultPackage = defaultPackageFor pkgs.stdenv.hostPlatform.system;
in
{
