...
# installer_state.cpp gets compiled in.
chmod -R u+w recipes/packages/system/reproos-installer/.repro/output 2>/dev/null || true
rm -rf recipes/packages/system/reproos-installer/.repro/output
rm -rf recipes/packages/system/reproos-installer/.repro/build
...
strings recipes/reproos-iso/build/de-rootfs/build/bin/repro 2>/dev/null | grep -c 'install-root' 2>&1 | tee -a "$LOG"
