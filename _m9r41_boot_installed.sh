#!/usr/bin/env bash
# M9.R.41 — Phase E: boot the installed disk + DE smoke.
#
# After _m9r41_install.sh completes (rc=0, all 6 phases incl. the new
# install-root rsync), this driver boots the installed disk (no ISO
# attached) and probes the DE binaries.  Captures each --version line.
#
# DISK defaults to /tmp/m9r41_install.qcow2 (the install driver's
# output).  Override via env if running against a different install.
set -uo pipefail
DISK="${DISK:-/tmp/m9r41_install.qcow2}"
BOOT_LOG="${BOOT_LOG:-/tmp/m9r41_boot_installed.log}"
TIMEOUT="${TIMEOUT:-360}"
...
BOOT_VARS=/tmp/m9r41_boot_ovmf_vars.fd
...
(
  ...
  echo "reproos"
  sleep 5
  echo "echo === M9R41_INSTALLED_DE_SMOKE_BEGIN ==="
  sleep 1
  echo "uname -a"
  sleep 2
  ...
  sleep 3
  echo "echo === M9R41_INSTALLED_DE_SMOKE_END ==="
  sleep 3
  echo "poweroff"
) > "$BOOT_FIFO" &

echo "=== M9.R.41 boot installed system (timeout ${TIMEOUT}s) ===" | tee "$BOOT_LOG"
...
if kill -0 $QPID 2>/dev/null; then
  echo "[m9r41-boot] ${TIMEOUT}s timeout, killing QEMU" | tee -a "$BOOT_LOG"
  kill -9 $QPID 2>/dev/null
fi
wait $QPID 2>/dev/null

echo ""
echo "=== M9.R.41 boot installed log (last 200 lines, ANSI stripped) ==="
sed 's/\x1B\[[0-9;]*[mK]//g' "$BOOT_LOG" | tail -200

echo ""
echo "=== smoke markers ==="
grep -E "M9R41_INSTALLED_DE_SMOKE|sway version|kwin|mutter|plasmashell|startplasma|sddm|SWAY_RC|KWIN_RC|MUTTER_RC|PLASMA_RC|STARTPLASMA_RC|SDDM_RC" "$BOOT_LOG" | tail -40
