...
suite "kernelSource — from-source recipe smoke test":
  ...
  test "makeFlags registers the exact production flag sequence":
    ...
    let declared = declaredBuildOptions("kernelSource")
    ...
    check buildBlockConstructors("kernelSource") == @["autotools_package"]
  test "makeFlags does not leak into the meson channel":
    ...
    check "meson_package" notin buildBlockConstructors("kernelSource")
