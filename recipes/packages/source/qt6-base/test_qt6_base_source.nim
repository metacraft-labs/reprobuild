...
suite "qt6BaseSource — from-source recipe smoke test":
  ...
  test "cmakeFlags registers the exact production flag sequence":
    ...
    let declared = declaredBuildOptions("qt6BaseSource")
    ...
    check buildBlockConstructors("qt6BaseSource") == @["cmake_package"]
  test "cmakeFlags does not leak into the meson channel":
    ...
    check "meson_package" notin buildBlockConstructors("qt6BaseSource")
  test "cmakeFlags does not leak into the configure channel":
    ...
    check "autotools_package" notin buildBlockConstructors("qt6BaseSource")
