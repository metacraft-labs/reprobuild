...
suite "mutterSource — from-source recipe smoke test":
  ...
  test "mesonOptions registers the exact production flag sequence":
    ...
    # silently change build behaviour (introspection on/off, profiler
    # on/off, tests on/off, debug on/off, native_backend on/off,
    # wayland on/off, x11 on/off, xwayland on/off, remote_desktop
    # on/off, release/debug).
    let flags = registeredBuildFlags("mutterSource", "", "meson")
  ...
  test "mesonOptions does not leak into the cmake channel":
    ...
    check registeredBuildFlags("mutterSource", "", "cmake") == emptyStrSeq
  ...
  test "mesonOptions does not leak into the configure channel":
    ...
    # ``./configure``).
    let emptyStrSeq: seq[string] = @[]
    check registeredBuildFlags("mutterSource", "", "configure") == emptyStrSeq
