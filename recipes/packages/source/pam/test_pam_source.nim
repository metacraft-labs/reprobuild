...
suite "pamSource — from-source recipe smoke test":
  ...
  test "configureFlags registers the exact production flag sequence":
    ...
    let declared = declaredBuildOptions("pamSource")
    ...
    check buildBlockConstructors("pamSource") == @["autotools_package"]
  test "configureFlags does not leak into the meson channel":
    ...
    check "meson_package" notin buildBlockConstructors("pamSource")
  test "configureFlags does not leak into the cmake channel":
    # Channel isolation: a recipe drives exactly ONE upstream build
    # system, so its ``build:`` block calls exactly one Layer-1
    # constructor. Options leaking into the cmake channel would
    # surface as a ``cmake_package(...)`` call here.
    check "cmake_package" notin buildBlockConstructors("pamSource")
