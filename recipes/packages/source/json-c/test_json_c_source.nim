...
suite "jsonCSource — from-source recipe smoke test":
  ...
  test "cmakeFlags registers the exact production flag sequence":
    ...
    # (shared/static, testing, apps, release/debug).
    let flags = registeredBuildFlags("jsonCSource", "", "cmake")
    check flags == ExpectedCmakeFlags
    check flags.len == 5

  test "cmakeFlags does not leak into the meson channel":
    # Cross-channel isolation — first place the M9.I per-channel
    # partitioning is exercised from the CMake side. A regression
    # that flattens the registries would surface the CMake flags
    # under the meson channel.
    let emptyStrSeq: seq[string] = @[]
    check registeredBuildFlags("jsonCSource", "", "meson") == emptyStrSeq

  test "cmakeFlags does not leak into the configure channel":
    # Cross-channel isolation #2 — guards against a regression that
    # merges the CMake + autotools channels (both produce ``-D``-shaped
    # arguments at a glance but the convention layer treats them as
    # disjoint inputs to ``cmake`` vs ``./configure``).
    let emptyStrSeq: seq[string] = @[]
    check registeredBuildFlags("jsonCSource", "", "configure") == emptyStrSeq
