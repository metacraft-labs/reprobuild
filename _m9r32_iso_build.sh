#!/usr/bin/env bash
# M9.R.32 ISO rebuild driver. Mirrors _m9r31_iso_build.sh.
set -uo pipefail
cd /opt/repro/reprobuild
LOG=/tmp/m9r32_iso_build.txt
...
nix-shell -p openssl patchelf gcc binutils gnumake autoconf automake libtool pkg-config gettext perl python3 bison flex xz gawk kmod cpio squashfsTools libisoburn grub2 mtools dosfstools curl --run "PATH=/opt/repro/reprobuild/build/bin:\$PATH LD_LIBRARY_PATH=$LD_LIBRARY_PATH repro build --daemon=off --tool-provisioning=from-source recipes/reproos-iso" > "$LOG" 2>&1
rc=$?
echo "rc=$rc"
tail -5 "$LOG"
