...
( cd "$WWW" && exec python3 -m http.server "$PORT" --bind 127.0.0.1 ) \
  >"$WORK/http.log" 2>&1 &
HTTP_PID=$!
# Poll rather than sleep-and-hope: a fixed sleep either wastes time or
# races, and a race here would look like "the repository was rejected".
i=0
while [ "$i" -lt 50 ]; do
...
if run_capture "$WORK/http-probe.log" curl -fsS -o /dev/null "$BASE/deb/dists/stable/InRelease"; then
  ok "local HTTP server serves InRelease over $BASE"
else
