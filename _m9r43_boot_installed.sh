#!/usr/bin/env bash
# M9.R.43 - Gap 4: boot the installed disk + DE smoke.
#
# After _m9r43_install.sh completes (rc=0, all 6 phases incl. the install-
# root rsync), this driver boots the installed disk (no ISO attached) and
# probes the DE binaries.  Captures each --version line.
#
# DISK defaults to /tmp/m9r43_install.qcow2 (the install driver's output).
set -uo pipefail
DISK="${DISK:-/tmp/m9r43_install.qcow2}"
BOOT_LOG="${BOOT_LOG:-/tmp/m9r43_boot_installed.log}"
TIMEOUT="${TIMEOUT:-420}"
...
BOOT_VARS=/tmp/m9r43_boot_ovmf_vars.fd
...
mkfifo "$BOOT_FIFO"
(
  # Wait through GRUB boot + multi-user.target + login prompt before
  # sending keystrokes.
  sleep 180
  echo "root"
  sleep 3
  echo "reproos"
  sleep 5
  echo "echo === M9R43_INSTALLED_DE_SMOKE_BEGIN ==="
  sleep 1
  echo "uname -a"
  sleep 2
  ...
  sleep 3
  echo "echo === M9R43_INSTALLED_DE_SMOKE_END ==="
  sleep 3
  echo "poweroff"
) > "$BOOT_FIFO" &

echo "=== M9.R.43 boot installed system (timeout ${TIMEOUT}s) ===" | tee "$BOOT_LOG"
...
if kill -0 $QPID 2>/dev/null; then
  echo "[m9r43-boot] ${TIMEOUT}s timeout, killing QEMU" | tee -a "$BOOT_LOG"
  kill -9 $QPID 2>/dev/null
fi
wait $QPID 2>/dev/null

echo ""
echo "=== M9.R.43 boot installed log (last 200 lines, ANSI stripped) ==="
sed 's/\x1B\[[0-9;]*[mK]//g' "$BOOT_LOG" | tail -200

echo ""
echo "=== smoke markers ==="
grep -E "M9R43_INSTALLED_DE_SMOKE|sway version|kwin|mutter|plasmashell|startplasma|sddm|SWAY_RC|KWIN_RC|MUTTER_RC|PLASMA_RC|STARTPLASMA_RC|SDDM_RC" "$BOOT_LOG" | tail -40
