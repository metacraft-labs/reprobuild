...
type
  ...
  DependencyPathSet* = object
    ...
    declarationDerived*: bool
      ## DA-1f — this report's entries were ASSEMBLED FROM A DECLARATION, not
      ## reported by a tool that looked at the action.
      ##
      ## WHY A REPORT HAS TO CARRY THIS. A depfile a real tool emits is an
      ## observation: `gcc -MD` writes the headers it actually opened. A
      ## depfile `fs.unmonitorableActionDepfile` emits is a literal built from
      ## the recipe author's `inputs` list — its own docstring says "nothing
      ## looked at the action to produce it". The two arrive at the engine as
      ## the same bytes in the same format and land in the same channel
      ## (`PathSetEvidence.depfileInputs`), which is a TERM OF THE
      ## ZERO-EVIDENCE GUARD. Without this flag that guard can be satisfied
      ## by a set that is 100% declaration-derived, which is the shape
      ## `../../../reprobuild-specs/Dependency-Observation-Attribution.md`
      ## rule 8 exists to stop.
      ##
      ## ATTRIBUTION, NOT SUPPRESSION (that document's §"Attribution, not
      ## suppression"): the entries are kept, in the same channel, and are
      ## still hashed and still invalidate the edge. All that changes is that
      ## a consumer asking "did anything OBSERVE this action" can now tell.
      ##
      ## THE ZERO VALUE IS THE ONE THAT CLAIMS LESS. `false` means "no
      ## generator stamp was found", which is what every tool-written depfile
      ## is and what every reader predating this field assumed of all of them.
      ## A reader that ignores the field therefore behaves exactly as before.

const
  DeclarationDerivedDepfileMarker* = "generated by unmonitorableActionDepfile"
    ## The generator stamp `repro_project_dsl`'s
    ## `unmonitorableActionDepfileText` writes into the first comment line of
    ## the depfile it assembles, and the needle this reader matches on.
    ##
    ## IT IS NOT SHARED WITH THE WRITER AS A CONSTANT, DELIBERATELY. A shared
    ## const would make the two sides agree by construction and grade nothing;
    ## the DSL lives one layer up and importing it here would invert the
    ## dependency. The agreement is instead GRADED, by a round-trip case that
    ## feeds `unmonitorableActionDepfileText`'s real output to
    ## `readRecognizedDependencyReport` and requires `declarationDerived` —
    ## see `tests/unit/t_unmonitorable_action_depfile_guards.nim`. If the
    ## writer's wording drifts, that case reddens; a shared const would have
    ## let the writer stop emitting a comment at all and stayed green.
...
proc parseMakeLikeText(path, text: string): DependencyPathSet =
  ...
  while i < normalized.len:
    ...
    of '$':
      ...
      sawRuleText = true
    of '#':
      # DA-1f — the comment is SKIPPED for the grammar and READ for its
      # provenance stamp. A make depfile has no field for "who wrote this",
      # so the one place a generator can say so is a comment, and the one
      # place a reader can notice is here. Scanned over the same slice the
      # grammar discards, so there is no second pass and no second reading
      # of the file.
      let commentStart = i
      while i < normalized.len and normalized[i] notin {'\n', '\r'}:
        inc i
      if not parsed.declarationDerived and
          normalized.find(DeclarationDerivedDepfileMarker, commentStart,
                          i - 1) >= 0:
        parsed.declarationDerived = true
      dec i
    of ':':
