...
package `python-dev`:
  provisioning:
    ...
    # tarball pinned to the same (3.12.13, build 20260610) pair. Ships the
    # interpreter plus the full Lib/ + include/ tree the PyO3 link step
    # needs (the Linux variant uses the standard POSIX layout under
    # `python/bin/` rather than the Windows `python/` root).
    #
    # The declared executable is ``python/bin/python3.12``, the one regular
    # file. ``python/bin/python3`` is a relative symlink to it, and tarball
    # realization refuses a declared executable reached through a symlink.
    # ``python3`` is still on PATH beside it in the same ``bin/`` directory.
    ...
    # Apple Silicon build. Ships the interpreter plus the full Lib/ +
    # include/ tree the PyO3 link step needs (POSIX layout under
    # `python/bin/`, matching the Linux entry — including ``python3``
    # being a symlink to the declared ``python3.12``).
