...
package stylus:
  provisioning:
    ...
    # macOS aarch64: same Node.js distribution as `node.nim`. Same
    # caveat as the Windows + Linux entries — stylus itself is npm-only;
    # this points at `bin/node` to satisfy build-time tool resolution.
