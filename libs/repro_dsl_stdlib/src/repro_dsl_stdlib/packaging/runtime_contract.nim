...
proc runtimeClosureScript*(dist: Distribution;
  ...
  # M1's N6. These names come from a RECIPE and are spliced into a `case`
  # pattern list in a generated shell script. Unquoted, a name containing
  # `)`, `|`, `;`, `$`, a backtick or whitespace does not merely fail to
  # match -- it closes the pattern and the rest becomes script, executed by
  # the staging edge with the builder's privileges. Two defences, because
  # each catches what the other cannot:
  #
  #   * REFUSE anything that is not a plausible shared-library leaf name.
  #     A build-graph-time `ValueError` naming the value, in the same shape
  #     as `types.validate`'s other refusals, rather than a script that
  #     misbehaves at staging time.
  #   * QUOTE what survives. `case` patterns may be quoted, and quoting
  #     removes their glob meaning, so `libfoo.so.1` matches the file
  #     called that and nothing else. Belt and braces: the filter is a
  #     judgement about what a leaf name looks like and the quoting is not
  #     a judgement about anything.
  const AllowedLeafChars = {'a'..'z', 'A'..'Z', '0'..'9', '.', '_', '+', '-'}
  var systemCases: seq[string] = @[]
  for name in dist.runtime.extraSystemLibraryLeafNames:
    if name.len > 0:
      for ch in name:
        if ch notin AllowedLeafChars:
          raise newException(ValueError,
            "runtime.extraSystemLibraryLeafNames: " &
            "'" & name & "' is not a shared-library leaf name (the " &
            "character '" & $ch & "' is not allowed). The value is " &
            "spliced into a generated shell script's `case` pattern " &
            "list; a name is a FILE NAME, never a path and never a " &
            "pattern.")
      systemCases.add(shellSingleQuote(name))
      let stem = libraryStem(name)
      if stem != name: systemCases.add(shellSingleQuote(stem))
  result = ""
  result.add("set -euf\n")
  result.add("# Generated by the reprobuild DSL packaging layer\n")
...
proc shebangRewriteScript*(source, output, interpreter: string): string =
  ## The POSIX-shell program that gives a staged ``crHelperScript`` a
  ## ``#!`` line the TARGET can resolve.
  ##
  ## This is the shebang half of what ``patchelf --set-interpreter``
  ## does for an ELF component, and it is a build-time SCRIPT for the
  ## same reason the closure walk is: the first line of the file is a
  ## property of a file some earlier edge may not have produced yet at
  ## the moment the graph is built, so it can only be read inside an
  ## ACTION. Reading it at graph-construction time and emitting the
  ## result through ``writeText`` would work for a payload that is
  ## checked in and silently stage the WRONG bytes for one that is
  ## generated.
  ##
  ## THE REFUSAL IS PART OF THE JOB. A component with no ``#!`` at all
  ## is not "nothing to rewrite": it is a file this role has declared
  ## executable and mode 0755, and ``execve`` on such a file hands it to
  ## the caller's shell or fails outright depending on the kernel. The
  ## build stops and names the file.
  ##
  ## Nothing here consults the BUILD host: ``interpreter`` is the
  ## target's path, and the script must NOT test it for existence. A
  ## cross-staged tree names ``/usr/bin/python3`` on a builder that has
  ## none, and a check that passed only on hosts resembling the target
  ## is exactly the ambient dependency this whole contract removes.
  result = ""
  result.add("set -eu\n")
  result.add("# Generated by the reprobuild DSL packaging layer\n")
  result.add("# (Distribution-And-Packaging.md " & "SECT" & "5, M1 N33). " &
    "Do not edit.\n")
  result.add("SRC=" & shellSingleQuote(source) & "\n")
  result.add("DST=" & shellSingleQuote(output) & "\n")
  result.add("INTERP=" & shellSingleQuote(interpreter) & "\n")
  result.add("mkdir -p -- \"$(dirname -- \"$DST\")\"\n")
  result.add("FIRST=$(head -n 1 -- \"$SRC\")\n")
  result.add("case $FIRST in\n")
  result.add("  '#!'*) ;;\n")
  result.add("  *)\n")
  result.add("    echo \"shebang: $SRC has no '#!' line (first line: " &
    "[$FIRST]); a crHelperScript is staged 0755 and an executable with " &
    "no interpreter is a file the target cannot run\" >&2\n")
  result.add("    exit 1 ;;\n")
  result.add("esac\n")
  # The old line is REPORTED, not silently replaced. The whole of N33 is
  # that nobody could see what the shipped first line was, and a build
  # log that names it turns the next occurrence into a grep.
  result.add("echo \"shebang: $SRC: [$FIRST] -> [#!$INTERP]\"\n")
  result.add("printf '%s\\n' \"#!$INTERP\" > \"$DST\"\n")
  result.add("tail -n +2 -- \"$SRC\" >> \"$DST\"\n")
  result.add("chmod 0755 -- \"$DST\"\n")
...
proc stageInstallTree*(dist: Distribution; variant: string;
  ...
  for component in dist.components:
    ...
    # ---- 1b. the SHEBANG, for the one role patchelf cannot touch ----
    #
    # The same rewrite as the ``--set-interpreter`` above, over the same
    # class of defect, for the role that was exempted from the tool that
    # did it. See ``ComponentRole.crHelperScript`` and
    # ``DistComponent.scriptInterpreter``: exempting this role from
    # patchelf exempted it from BOTH of patchelf's jobs, and only one of
    # them was unwanted. M1's N33 is the other one, shipped in five
    # formats.
    #
    # ``validate`` has already refused an absent, relative or store-path
    # interpreter, so by here the value is a target path. Windows has no
    # shebang mechanism at all -- the loader does not read the first
    # line of a file -- so the role is staged verbatim there, which is
    # why this is gated on ``posix`` and not on ``toLinux``: a Darwin
    # target reads ``#!`` exactly as Linux does.
    if posix and component.role == crHelperScript:
      let fixed = genRoot & "/" & sanitizeIdPart(variant) & "-" &
        sanitizeIdPart(prefixRel) & ".shebang"
      let edge = sh_module.shell(
        shebangRewriteScript(component.buildPath, fixed,
          component.scriptInterpreter),
        actionId = idPrefix & "shebang-" & sanitizeIdPart(prefixRel),
        # ...and the directory the rewritten copy lands in. Same
        # ``genRootEdge`` the patchelf edge needs, and for the same
        # reason -- except that this script creates the parent itself
        # too, because a shell can and patchelf cannot. Both, because
        # ordering after the edge that owns the directory is what keeps
        # two producers over one Distribution from racing on it.
        after = component.producedBy & @[genRootEdge],
        # The file the rewrite READS. Declared, so a changed helper
        # re-stages: the whole point of doing this in an action rather
        # than at graph-construction time.
        extraInputs = @[component.buildPath],
        extraOutputs = @[fixed])
      declareProducerTool(site, edge.id, ShSelector)
      noteSelector(ShSelector)
      # coreutils: ``head``, ``tail``, ``chmod``, ``dirname``, ``mkdir``.
      # Reached through the ``install`` executable's package, exactly as
      # the closure edge reaches it.
      declareProducerTool(site, edge.id, InstallSelector)
      noteSelector(InstallSelector)
      payloadSource = fixed
      payloadAfter = @[edge]
