...
proc profileBuildActionToBuildAction*(pba: ProfileBuildAction):
  ...
  if pba.argv.len == 0:
    raise newException(ValueError,
      ...
      "intent)")
  # Profile-scope action edges declare their inputs + outputs
  # explicitly; the engine's automatic-monitor dependency gathering
  # (which shells out to ``repro-fs-snoop``) is not appropriate here
  # — the apply driver has no fs-snoop binary on PATH and the
  # action's input/output set is already authoritative. Tag the
  # action with ``dgNoRuntimeDependencies`` so the engine fingerprints
  # against the declared inputs and skips the monitor entirely.
  result = action(
    id = pba.id,
    argv = pba.argv,
    cwd = pba.cwd,
