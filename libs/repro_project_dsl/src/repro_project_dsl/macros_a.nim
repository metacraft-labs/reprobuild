...
proc parseExecutable(packageName: string; node: NimNode): ExecutableDef =
  ## Accepts the same two shapes ``parseLibrary`` does — and says so here
  ## because ``parseLibrary``'s own docstring already claims to "mirror
  ## parseExecutable" on exactly this point:
  ##
  ##   ``executable foo``                — bare command, no body.
  ##   ``executable foo:`` + indented body — ``name:`` / ``lockFile`` /
  ##                                         ``cli:`` / the implicit-target
  ##                                         hook / ``discard``.
  ##
  ## The bare form used to CRASH the macro. ``let body = node[2]`` ran
  ## unconditionally, and a bare ``executable foo`` parses to a two-child
  ## ``nnkCommand`` — so the author got ``Error: index 2 not in 0 .. 1``
  ## pointing into ``macros_a.nim``, with nothing naming their own
  ## declaration or telling them a body was required. ``parseLibrary``
  ## has had the arity guard since M12 and ``library foo`` is covered by
  ## ``test_library_macro.nim``'s "Case 1"; the executable side simply
  ## never grew either.
  ##
  ## NOTE for recipe authors, because the guard makes the bare form parse
  ## and NOT because it makes it advisable: ``binaryName`` defaults to the
  ## Nim IDENT, so a bare ``executable ctCli`` declares an artifact named
  ## ``ctCli``. Whenever the on-disk basename differs from the ident —
  ## hyphenated or snake_case names, which is most of them — write the
  ## explicit ``name:``.
  let loc = lineFile(node)
  if node.len < 2:
    error("executable expects a name", node)
