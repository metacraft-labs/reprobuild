...
suite "t_engine_typed_output_payload_codec_round_trip":
  ...
  test "older v11 payload decodes with empty typed-output list":
    ...
    # Truncate 19 trailing bytes: 4 for the empty typedOutputs count
    ...
    # requiresElevation sentinel byte (the v19 addition). All seven
  ...
  test "older v16 payload decodes with empty toolIdentityRefs (M9.N Batch B)":
    ...
    # version field down to 16 and trimming the trailing 5 bytes (4 for
    # the v17 toolIdentityRefs length-prefix + 1 for the v19 Windows-
    # System-Resources Phase E requiresElevation sentinel byte).
    # v16-and-earlier payloads MUST decode with both fields at their
    # inert defaults (empty ``toolIdentityRefs`` + ``requiresElevation
    # = false``) so legacy artefacts keep working.
    let action = BuildActionDef(
      id: "v16-legacy",
      call: publicCliCall("pkg", "exe", "build",
        "pkg.exe.build", @[]),
