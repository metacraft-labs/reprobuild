...
/* The POSIX arm fills the three seams: the real lifecycle and a pthread mutex. */
static pthread_mutex_t rb_hcr_padded_mutex = PTHREAD_MUTEX_INITIALIZER;
static int rb_hcr_platform_pending_active(void) { return rb_hcr_pending.active; }
static void rb_hcr_platform_apply_pending(void) {
  rb_hcr_pending.active = 0;
  rb_hcr_run_lifecycle(&rb_hcr_pending);
  rb_hcr_request_release(&rb_hcr_pending);
}
static void rb_hcr_platform_padded_lock(void) { pthread_mutex_lock(&rb_hcr_padded_mutex); }
static void rb_hcr_platform_padded_unlock(void) { pthread_mutex_unlock(&rb_hcr_padded_mutex); }
static int rb_hcr_platform_aligned_alloc(void **out, size_t alignment, size_t size) {
  return posix_memalign(out, alignment, size);
}
static void rb_hcr_platform_aligned_free(void *ptr) { free(ptr); }
