...
proc dlLinkFlags(): seq[string] =
  ## Link flags needed for the runtime's `dladdr`-based name resolution.
  ##
  ## On Linux, `dladdr` lives in libdl (`-ldl`) AND only resolves names that are
  ## in the binary's DYNAMIC symbol table — a normal executable's own
  ## (non-exported) `static`/local functions are NOT there, so `dladdr` would
  ## return no name for every instrumented entry and the executed-set log would
  ## stay empty. `-rdynamic` (`-Wl,--export-dynamic`) exports all symbols into
  ## `.dynsym` so `dladdr(this_fn).dli_sname` resolves the program's own
  ## functions. On macOS `dladdr` resolves Mach-O local symbols without an
  ## explicit export flag and there is no separate libdl, so neither flag is
  ## needed (and `-ldl` would be an error). Branch by host OS.
  when defined(macosx):
    @[]
  else:
    @["-ldl", "-rdynamic"]
